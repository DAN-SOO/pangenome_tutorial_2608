...
> 공용 conda를 쓰는 서버에서 `Operation not permitted` 오류가 나면 패키지 캐시를
> 개인 폴더로 돌리세요: `export CONDA_PKGS_DIRS=$HOME/conda/pkgs`
...
WORK가 의도한 곳인지만 보면 됩니다. 공유 데이터 폴더 안이면 경고가 나옵니다.
